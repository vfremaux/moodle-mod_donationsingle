 ...
 */
	...
	$sqlcount = "
        SELECT 
            COUNT(*)
        FROM 
            {donationsingle_donator} as e
        WHERE 
            e.donationid = ? 
        GROUP BY
            e.donationid
    ";
    $numrecords = $DB->count_records_sql($sqlcount, array($donationsingle->id));
?>
<center>
<?php
...
/// get extra query parameters from flexible_table behaviour
    ...
    $donators = $DB->get_records_sql($sql, array($donationsingle->id), $table->get_page_start(), $table->get_page_size());
    $space = ' ';
    if (!empty($donators)){
        ...
        foreach ($donators as $donator){
    		...
    		$userpic->user = $DB->get_record('user', array('id' => $donator->userid));
    		$userpic->courseid = $cm->id;
    		*/
    		$picture = $OUTPUT->user_picture($donator);
    		$fullname = "<a href= \"view.php?id={$cm->id}&amp;view=view&amp;page=history&amp;userid={$donator->id}\">".fullname($donator)."</a>";
