...
function donationsingle_add_instance($donationsingle) {
    ...
    return $DB->insert_record("donationsingle", $donationsingle);
}
...
function donationsingle_update_instance($donationsingle) {
    ...
    if (!isset($donationsingle->intro)) $donationsingle->intro = '';
    $donationsingle->introformat = 0;

    return $DB->update_record("donationsingle", $donationsingle);
}
...
function donationsingle_delete_instance($id) {
    ...
    if (! $donationsingle = $DB->get_record('donationsingle', array('id' => "$id"))) {
        return false;
    }
    $result = true;

    # Delete any dependent records here #
	$DB->delete_records('donationsingle', array('id' => $id));
	$DB->delete_records('donationsingle_donator', array('donationid' => $id));
	$DB->delete_records('donationsingle_history', array('donationid' => $id));
...
function donationsingle_cron () {
	...
	if ($donations = $DB->get_records('donationsingle', null)){
		...
		foreach($donations as $ds){
			...
			$DB->update_record('donationsingle', addslashes_recursive($ds));
		}
	}
    return true;
}
...
function donationsingle_get_participants($donationid) {
	...
	$participants = $DB->get_records('donationsingle_donator', array('donationid' => $donationid));
